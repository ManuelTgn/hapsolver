...
@pytest.mark.parametrize(
    ...
    [
        (
            Sequence("ATGC"),
            Coordinate("chr1", 10, 13, 0),
            4,
            ">chr1:10-13\nATGC",
            "<Region object; region=chr1:10-13>",
        ),  # Happy path
        (
            Sequence(""),
            Coordinate("chrX", 100, 100, 0),
            0,
            ">chrX:100-100\n",
            "<Region object; region=chrX:100-100>",
        ),  # Empty sequence
    ],
    ids=["non_empty_sequence", "empty_sequence"],
)
def test_region_init_len_str_repr(
    sequence: Sequence,
    coord: Coordinate,
    expected_len: int,
    expected_str: str,
...
@pytest.mark.parametrize(
    ...
    [
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 12, 18, 0),
            True,
        ),  # Happy path, full containment
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 10, 20, 0),
            True,
        ),  # Edge case, identical regions
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 12, 22, 0),
            False,
        ),  # Edge case, partial overlap, extending beyond end
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 8, 18, 0),
            False,
        ),  # Edge case, partial overlap, extending before start
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr2", 12, 18, 0),
            False,
        ),  # Edge case, different contigs
    ],
    ids=[
        "full_containment",
        "identical_regions",
        "extends_beyond_end",
        "extends_before_start",
        "different_contigs",
    ],
)
def test_contain(
    region1_coord: Coordinate, region2_coord: Coordinate, expected_contain: bool
) -> None:
    # Arrange
    region1 = Region(
        Sequence("A" * (region1_coord.stop - region1_coord.start + 1)), region1_coord
    )
    region2 = Region(
        Sequence("A" * (region2_coord.stop - region2_coord.start + 1)), region2_coord
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 15, 25, 0),
            True,
        ),  # Happy path, partial overlap
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 10, 20, 0),
            True,
        ...
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 21, 30, 0),
            False,
        ),  # Edge case, no overlap
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr2", 15, 25, 0),
            False,
        ),  # Edge case, different contigs
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 5, 10, 0),
            True,
        ),  # Edge case, region2 starts before region1 but overlaps at the start
        (
            Coordinate("chr1", 10, 20, 0),
            Coordinate("chr1", 20, 25, 0),
            True,
        ),  # Edge case, region2 starts at the end of region1 and extends beyond
    ],
    ids=[
        "partial_overlap",
        "identical_regions",
...
def test_overlap(
    region1_coord: Coordinate, region2_coord: Coordinate, expected_overlap: bool
) -> None:
    # Arrange
    region1 = Region(
        Sequence("A" * (region1_coord.stop - region1_coord.start + 1)), region1_coord
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [Region(Sequence("ATGC"), Coordinate("chr1", 10, 13, 0))],
            1,
            "<RegionList object; regions=1>",
        ),  # Single region
        (
            [
                Region(Sequence("ATGC"), Coordinate("chr1", 10, 13, 0)),
                Region(Sequence("GCTA"), Coordinate("chr2", 20, 23, 0)),
            ],
            2,
            "<RegionList object; regions=2>",
        ),  # Multiple regions
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            1,
            Region(Sequence("GCTA"), Coordinate("chr2", 20, 23, 0)),
        ),  # Multiple regions, accessing second region
        (
            [
                Region(Sequence("ATGC"), Coordinate("chr1", 10, 13, 0)),
                Region(Sequence("GCTA"), Coordinate("chr2", 20, 23, 0)),
                Region(Sequence("AAAA"), Coordinate("chr1", 1, 4, 0)),
            ],
            slice(1, 3),
            [
                Region(Sequence("GCTA"), Coordinate("chr2", 20, 23, 0)),
        ...
        ),  # Multiple regions, accessing a slice
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ),  # Extending with a single region
        (
            [],
            [Region(Sequence("ATGC"), Coordinate("chr1", 10, 13, 0))],
            [Region(Sequence("ATGC"), Coordinate("chr1", 10, 13, 0))],
