from hapsolver.variant import VariantRecord, _assign_vtype, _compute_id, _adjust_multiallelic, _genotypes_to_samples
...
@pytest.mark.parametrize(
    ...
    ids=["simple_snp", "deletion", "insertion", "mnp", "larger_deletion", "larger_insertion"]
)
...
def test_adjust_multiallelic(ref, alt, pos, expected_ref, expected_alt, expected_pos):
    ...
    # Assert
    assert (result_ref, result_alt, result_pos) == (expected_ref, expected_alt, expected_pos)


@pytest.mark.parametrize(
    "genotypes, samples, allelesnum, phased, expected",
    [
        (["1|0", "0|1"], ["sample1", "sample2"], 1, True, [({"sample1"}, {"sample2"})]),  # Phased het
        (["1/0", "0/1"], ["sample1", "sample2"], 1, False, [({"sample1", "sample2"}, set())]),  # Unphased het
        (["1|0", "0|0"], ["sample1", "sample2"], 1, True, [({"sample1"}, set())]),  # Phased with one hom ref
        (["1/0", "0/0"], ["sample1", "sample2"], 1, False, [({"sample1"}, set())]),  # Unphased with one hom ref
        (["0|0", "0|0"], ["sample1", "sample2"], 1, True, [(set(), set())]),  # Phased all hom ref
        (["0/0", "0/0"], ["sample1", "sample2"], 1, False, [(set(), set())]),  # Unphased all hom ref
        (["1|1", "1|0", "0|1"], ["s1", "s2", "s3"], 1, True, [({"s1", "s2"}, {"s1", "s3"})]),  # Phased all genotypes
        (["1/1", "1/0", "0/1"], ["s1", "s2", "s3"], 1, False, [({"s1", "s2", "s3"}, set())]),  # Unphased all genotypes
        (["1|0", "2|1", "0|2"], ["s1", "s2", "s3"], 2, True, [({"s1"}, {"s2"}), ({"s2"}, {"s3"})]),  # Phased multiallelic
        (["1/0", "2/1", "0/2"], ["s1", "s2", "s3"], 2, False, [({"s1", "s2"}, set()), ({"s3", "s2"}, set())]),  # Unphased multiallelic
...
class TestVariantRecord:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["chrX", "5000", ".", "AG", "A,AC", ".", "PASS", ".", ".", "1/0", "0/2"],
                ["sample1", "sample2"],
                False,
                "chrX",
                5000,
    ...
    def test_read_vcf_line(self, variant, samples, phased, chrom, position, ref, alt, allelesnum, vtype, filter, vid, samples_result):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("chr1", 100, "A", ["T"], '<VariantRecord object; variant="chr1 100 A T">'),  # Simple SNP
            ("chrX", 5000, "AG", ["A"], '<VariantRecord object; variant="chrX 5000 AG A">'),  # Deletion
        ],
        ids=["simple_snp", "deletion"]
